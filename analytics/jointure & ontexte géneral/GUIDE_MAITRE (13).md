...
⚠️ **Pas encore fait** : le notebook de pipeline d'entraînement lit encore les anciens chemins (`dataset_train_produits`/`dataset_a_scorer`) et l'ancien cadrage à un seul modèle. Il doit être mis à jour pour consommer les deux CSV/parquets nettoyés produits par `EDA_final.ipynb` (un par `DATASET_CIBLE`) et dupliquer/adapter les étapes d'entraînement pour les deux modèles.
...
- [ ] **Notebook de pipeline d'entraînement mis à jour** pour entraîner les deux modèles séparément à partir des sorties de `EDA_final.ipynb` (au lieu d'un seul multi-classes) — **à faire**
